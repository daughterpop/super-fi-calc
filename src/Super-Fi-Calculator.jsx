      ...
      <div className="w-full" ref={wizardRef}>
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 sm:p-5 mb-6">
            <h3 className="text-lg font-bold text-amber-900 mb-1">Your Stewardship Legacy</h3>
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-5 sm:p-8">
          ...
          {step === 3 && (
            <div className="space-y-5">
              ...
              {buyingVehicle && vehicles.map((v, i) => (
                <div key={v.id} className="grid grid-cols-2 gap-3 p-3 bg-gray-50 rounded-xl">
                  <div><label className="block text-xs font-medium mb-1">Amount</label>
                    <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm">$</span>
                      <MoneyField value={v.amount} onChange={(val) => updateVehicle(i, 'amount', val)} className="w-full pl-7 pr-3 py-2 border rounded-lg" ariaLabel="Vehicle amount" /></div></div>
                  <div><label className="block text-xs font-medium mb-1">Year</label>
                    <input type="number" value={v.year} onChange={(e) => updateVehicle(i, 'year', parseInt(e.target.value) || 0)} className="w-full px-3 py-2 border rounded-lg" /></div>
                </div>
              ))}
              ...
              {hasOtherExpenses && otherExpenses.map((e, i) => (
                <div key={i} className="grid grid-cols-2 gap-3 p-3 bg-gray-50 rounded-xl">
                  <div><label className="block text-xs font-medium mb-1">Amount</label>
                    <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm">$</span>
                      <MoneyField value={e.amount} onChange={(val) => updateOtherExpense(i, 'amount', val)} className="w-full pl-7 pr-3 py-2 border rounded-lg" ariaLabel="Other expense amount" /></div></div>
                  <div><label className="block text-xs font-medium mb-1">Year</label>
                    <input type="number" value={e.year} onChange={(ev) => updateOtherExpense(i, 'year', parseInt(ev.target.value) || 0)} className="w-full px-3 py-2 border rounded-lg" /></div>
                </div>
              ))}
            </div>
          )}
          <div className="mt-8 flex justify-between">
            {step > 0 ? <button onClick={() => setStep(step - 1)} className="px-5 py-2.5 text-sm font-medium text-gray-600 hover:text-gray-900">Back</button> : <div />}
            <button onClick={nextStep} className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl text-sm">{step < 3 ? 'Next' : 'See results'}</button>
          </div>
        </div>
      </div>
    </div>
