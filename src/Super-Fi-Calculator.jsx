        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-5 sm:p-8">
          ...
          {step === 2 && (
            <div className="space-y-5">
              ...
              <label className="flex items-center gap-2"><input type="checkbox" checked={payingForCollege} onChange={(e) => setPayingForCollege(e.target.checked)} /><span className="font-medium">Paying for college</span></label>
              {payingForCollege && (
                <>
                  <div className="flex justify-between"><span className="text-sm text-gray-600">Add each child</span><button onClick={() => setKids([...kids, { id: Date.now(), age: 5, annualTuition: 46000 }])} className="text-sm text-emerald-600 font-medium">+ Add Child</button></div>
                  {kids.map((kid, idx) => (
          ...
          {step === 3 && (
            <div className="space-y-5">
              ...
              <label className="flex items-center gap-2"><input type="checkbox" checked={buyingVehicle} onChange={(e) => setBuyingVehicle(e.target.checked)} /><span className="font-medium">Buying vehicles</span></label>
              {buyingVehicle && (<>
                <div className="flex justify-between"><span className="text-sm text-gray-600">Add planned vehicles</span><button onClick={() => setVehicles([...vehicles, { id: Date.now(), amount: 50000, year: new Date().getFullYear() + 1 }])} className="text-sm text-emerald-600 font-medium">+ Add Vehicle</button></div>
                {vehicles.map((v, idx) => (
