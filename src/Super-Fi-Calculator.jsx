    ...
    <div className="w-full max-w-2xl mb-6 sm:mb-8 bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl p-4 sm:p-6 shadow-sm border border-green-100">
      <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-1">Ready to Start Investing?</h3>
          ...
          <p className="text-xs text-gray-500 mt-2 sm:mt-3">Commission-free trading • Easy to use • Perfect for beginners</p>
        </div>
      </div>
    </div>
  );
    ...
    return (
      <div className="w-full">
        <div className="max-w-6xl mx-auto">
          <button onClick={() => setShowResults(false)} className="mb-4 sm:mb-6 flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900 transition-colors">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-4 sm:mb-6">
            <div className="bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-gray-100">
              ...
              <div className="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-indigo-600 to-blue-600 bg-clip-text text-transparent mb-3">{results.fiProgress.toFixed(1)}%</div>
              <div className="w-full bg-gray-100 rounded-full h-2">
                <div className="bg-gradient-to-r from-indigo-600 to-blue-600 h-2 rounded-full transition-all duration-1000" style={{ width: `${Math.min(results.fiProgress, 100)}%` }} />
              </div>
            </div>
            <div className="bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-gray-100">
              <div className="text-sm font-medium text-gray-500 mb-2">Current Savings</div>
              <div className="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">${(currentSavings / 1000000).toFixed(2)}M</div>
              <div className="text-xs text-gray-500">Target: ${(results.currentTotalFI / 1000000).toFixed(2)}M</div>
            </div>
            <div className="bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-gray-100">
              <div className="text-sm font-medium text-gray-500 mb-2">Time to FI</div>
              <div className="text-2xl sm:text-3xl font-bold text-green-600">{results.yearsFIReached !== null ? `${results.yearsFIReached} years` : 'Off track'}</div>
          ...
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl p-4 sm:p-6 shadow-sm border border-green-100 mb-4 sm:mb-6">
            <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-1">Ready to Start Investing?</h3>
                <p className="text-sm text-gray-600 mb-3 sm:mb-4">Get free stocks worth up to $200 when you join Robinhood and make your first investment.</p>
                <a href="https://join.robinhood.com/dustinh-1bff5a" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 bg-green-600 text-white font-semibold rounded-xl hover:bg-green-700 transition-all shadow-md hover:shadow-lg text-sm sm:text-base">
                  Claim Your Free Stock <ArrowRight size={18} />
                </a>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
              {results.milestones.map((milestone, idx) => (
                <div key={idx} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 border border-gray-100">
                  <div className={`w-8 h-8 shrink-0 rounded-full flex items-center justify-center ${milestone.achieved ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                    {milestone.achieved ? <Check size={16} /> : <div className="w-2 h-2 rounded-full bg-gray-300" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className={`text-sm font-medium truncate ${milestone.achieved ? 'text-gray-900' : 'text-gray-400'}`}>{milestone.name}</div>
                    <div className="text-xs text-gray-500">${(milestone.target / 1000).toFixed(0)}k</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-gradient-to-br from-amber-50 via-white to-rose-50 border border-amber-200 rounded-3xl p-5 sm:p-8 mb-6 sm:mb-8 shadow-xl relative overflow-hidden">
            ...
            <div className="prose prose-amber max-w-none mb-6 sm:mb-8">
              <p className="text-base sm:text-lg italic leading-relaxed text-amber-800">"Well done, good and faithful servant. You have been faithful with a few things; I will put you in charge of many things." — Matthew 25:21</p>
              <p className="mt-4 sm:mt-6 text-sm sm:text-base text-amber-700">By God's grace, you are on track to achieve Financial Independence at age <span className="font-semibold text-amber-900">{results.fiAge || 'XX'}</span>. This is not merely wealth for comfort, but a sacred legacy of stewardship.</p>
            </div>
            <button onClick={handleShareLegacy} className="w-full flex items-center justify-center gap-3 bg-gradient-to-r from-amber-600 to-rose-600 hover:from-amber-700 hover:to-rose-700 text-white font-semibold py-3.5 sm:py-4 px-6 sm:px-8 rounded-2xl transition-all shadow-lg active:scale-[0.985] text-sm sm:text-base">
              <Share2 size={20} /> Bless &amp; Share This Legacy
            </button>
          </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {results.projections.slice(0, 25).map((proj, idx) => (
                    <tr key={idx} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-3 sm:px-4 py-3 text-sm text-right font-semibold text-gray-900">${(proj.balance / 1000000).toFixed(2)}M</td>
                      <td className="px-3 sm:px-4 py-3 text-sm text-right text-indigo-600">${(proj.totalFINumber / 1000000).toFixed(2)}M</td>
                      <td className="px-3 sm:px-4 py-3 text-sm text-right text-orange-600">{proj.collegeExpense > 0 ? `-$${(proj.collegeExpense / 1000).toFixed(0)}k` : '-'}</td>
                      <td className="px-3 sm:px-4 py-3 text-center">
                        {proj.balance >= proj.totalFINumber ? (
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">FI ✓</span>
                        ) : (
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            {steps.map((s, idx) => (
              <React.Fragment key={idx}>
                ...
                {idx < steps.length - 1 && <div className={`w-4 sm:w-8 h-0.5 shrink-0 ${idx < step ? 'bg-indigo-600' : 'bg-gray-200'}`} />}
              </React.Fragment>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl border border-gray-100 p-5 sm:p-8 md:p-10">
          {step === 0 && (
            <div className="space-y-5 sm:space-y-6">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Annual Expenses <span className="text-gray-500 font-normal text-xs">(in today's dollars)</span></label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">$</span>
                  <input type="number" value={annualExpenses} onChange={(e) => setAnnualExpenses(parseFloat(e.target.value) || 0)} className="w-full pl-8 pr-4 py-3 sm:py-4 text-base sm:text-lg border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:outline-none transition-colors" />
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" value={currentAge} onChange={(e) => setCurrentAge(parseInt(e.target.value) || 0)} className="w-full px-4 py-3 sm:py-4 text-base sm:text-lg border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:outline-none transition-colors" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Current Savings</label>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">$</span>
                    <input type="number" value={currentSavings} onChange={(e) => setCurrentSavings(parseFloat(e.target.value) || 0)} className="w-full pl-8 pr-4 py-3 sm:py-4 text-base sm:text-lg border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:outline-none transition-colors" />
                  </div>
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">$</span>
                  <input type="number" value={annualSavings} onChange={(e) => setAnnualSavings(parseFloat(e.target.value) || 0)} className="w-full pl-8 pr-4 py-3 sm:py-4 text-base sm:text-lg border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:outline-none transition-colors" />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Expected Rate of Return</label>
                  <div className="relative">
                    <input type="number" step="0.1" value={rateOfReturn} onChange={(e) => setRateOfReturn(parseFloat(e.target.value) || 0)} className="w-full px-4 py-3 sm:py-4 text-base sm:text-lg border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:outline-none transition-colors" />
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type="number" step="0.1" value={inflationRate} onChange={(e) => setInflationRate(parseFloat(e.target.value) || 0)} className="w-full px-4 py-3 sm:py-4 text-base sm:text-lg border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:outline-none transition-colors" />
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500">%</span>
                  </div>
                </div>
              </div>
          ...
          {step === 1 && (
            <div className="space-y-5 sm:space-y-6">
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={hasMortgage} onChange={(e) => setHasMortgage(e.target.checked)} className="w-5 h-5 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
              ...
              {hasMortgage && (
                <>
                  <div>
                    ...
                    <div className="relative">
                      <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">$</span>
                      <input type="number" value={mortgageBalance} onChange={(e) => setMortgageBalance(parseFloat(e.target.value) || 0)} className="w-full pl-8 pr-4 py-3 sm:py-4 text-base sm:text-lg border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:outline-none transition-colors" />
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="relative">
                        <input type="number" step="0.01" value={mortgageRate} onChange={(e) => setMortgageRate(parseFloat(e.target.value) || 0)} className="w-full px-4 py-3 sm:py-4 text-base sm:text-lg border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:outline-none transition-colors" />
                        <span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500">%</span>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">$</span>
                        <input type="number" value={monthlyPayment} onChange={(e) => setMonthlyPayment(parseFloat(e.target.value) || 0)} className="w-full pl-8 pr-4 py-3 sm:py-4 text-base sm:text-lg border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:outline-none transition-colors" />
                      </div>
                    </div>
                  </div>
                </>
          ...
          {step === 2 && (
            <div className="space-y-5 sm:space-y-6">
              ...
              <label className="flex items-start sm:items-center gap-3 cursor-pointer p-3 sm:p-4 border-2 border-gray-200 rounded-xl hover:border-indigo-300 transition-colors">
                <input type="checkbox" checked={payingForCollege} onChange={(e) => setPayingForCollege(e.target.checked)} className="w-5 h-5 mt-0.5 sm:mt-0 shrink-0 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
              ...
              {payingForCollege && (
                <>
                  <div className="flex justify-between items-center gap-2">
                    ...
                    <button onClick={addKid} className="text-sm font-medium text-indigo-600 hover:text-indigo-700 whitespace-nowrap">+ Add Child</button>
                  </div>
                  {kids.map((kid, idx) => (
                    <div key={kid.id} className="p-3 sm:p-4 border-2 border-gray-100 rounded-xl space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        {kids.length > 1 && <button onClick={() => removeKid(idx)} className="text-sm text-red-600 hover:text-red-700">Remove</button>}
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-xs font-medium text-gray-600 mb-1">Age</label>
                          <input type="number" value={kid.age} onChange={(e) => updateKid(idx, 'age', parseInt(e.target.value) || 0)} className="w-full px-3 py-2.5 sm:py-3 border-2 border-gray-200 rounded-lg focus:border-indigo-500 focus:outline-none" />
                        </div>
                        <div>
                          <label className="block text-xs font-medium text-gray-600 mb-1">Annual Tuition</label>
                          <div className="relative">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm">$</span>
                            <input type="number" value={kid.annualTuition} onChange={(e) => updateKid(idx, 'annualTuition', parseFloat(e.target.value) || 0)} className="w-full pl-7 pr-3 py-2.5 sm:py-3 border-2 border-gray-200 rounded-lg focus:border-indigo-500 focus:outline-none" />
                          </div>
                        </div>
                      </div>
                    </div>
          ...
          {step === 3 && (
            <div className="space-y-5 sm:space-y-6">
              ...
              <label className="flex items-center gap-3 cursor-pointer p-3 sm:p-4 border-2 border-gray-200 rounded-xl hover:border-indigo-300 transition-colors">
                <input type="checkbox" checked={buyingVehicle} onChange={(e) => setBuyingVehicle(e.target.checked)} className="w-5 h-5 shrink-0 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                <span className="text-base sm:text-lg font-medium text-gray-700">Vehicle purchase(s)</span>
              </label>
              {buyingVehicle && vehicles.map((vehicle, idx) => (
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="number" value={vehicle.amount} onChange={(e) => updateVehicle(idx, 'amount', parseFloat(e.target.value) || 0)} className="w-full pl-7 pr-3 py-2.5 sm:py-3 border-2 border-gray-200 rounded-lg focus:border-indigo-500 focus:outline-none" />
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">Year</label>
                      <input type="number" value={vehicle.year} onChange={(e) => updateVehicle(idx, 'year', parseInt(e.target.value) || 0)} className="w-full px-3 py-2.5 sm:py-3 border-2 border-gray-200 rounded-lg focus:border-indigo-500 focus:outline-none" />
                    </div>
                  </div>
                </div>
              ))}
              <label className="flex items-center gap-3 cursor-pointer p-3 sm:p-4 border-2 border-gray-200 rounded-xl hover:border-indigo-300 transition-colors">
                <input type="checkbox" checked={hasOtherExpenses} onChange={(e) => setHasOtherExpenses(e.target.checked)} className="w-5 h-5 shrink-0 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
          ...
          <div className="flex flex-col-reverse sm:flex-row gap-3 mt-6 sm:mt-8 pt-5 sm:pt-6 border-t border-gray-100">
            {step > 0 && (
              <button onClick={prevStep} className="flex items-center justify-center gap-2 px-6 py-3 rounded-xl border-2 border-gray-200 text-gray-700 font-medium hover:bg-gray-50 transition-colors w-full sm:w-auto">
            ...
            {step === steps.length - 1 ? (
              <div className="flex-1 space-y-3 w-full">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Email address to see your results</label>
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:outline-none transition-colors" />
                </div>
                <button onClick={nextStep} disabled={!email.includes('@')} className="w-full flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 text-white font-semibold hover:from-indigo-700 hover:to-blue-700 transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">
                  See Your Results <ArrowRight size={20} />
                </button>
              </div>
            ) : (
              <button onClick={nextStep} className="flex-1 flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 text-white font-semibold hover:from-indigo-700 hover:to-blue-700 transition-all shadow-lg w-full">
