    ...
    return (
      <div className="w-full" ref={wizardRef}>
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-xs sm:text-sm text-gray-500 text-center mb-4 max-w-2xl mx-auto">Numbers use total investments only (house equity excluded). Mortgage balance is added to the FI target, not treated as an asset. “Time to FI” is the first year the portfolio stays above the inflation-adjusted target for every year after.</p>
          <p className="text-sm font-semibold text-gray-900 text-center mb-2">What to do next</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
            <Link
              to="/blog/how-to-get-started-on-your-fi-path"
              className="bg-white border border-emerald-100 rounded-xl p-4 hover:border-emerald-300 hover:bg-emerald-50/40 transition"
            >
              <p className="font-semibold text-sm text-gray-900">Read the sequence</p>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-3xl p-5 mb-6">
            ...
            <p className="text-sm text-amber-700 mb-4">By God's grace, on track for Financial Independence at age <span className="font-semibold">{results.fiAge || 'XX'}</span> — margin for family, Church, and the work that lasts.</p>
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-5 sm:p-8">
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div><label htmlFor="fi-current-savings" className="block text-sm font-semibold mb-2">Total investments</label>
                  ...
                  <p className="text-xs text-gray-500 mt-1">$1.35M invested today — exclude house equity</p>
                </div>
              </div>
              <div>
                <label htmlFor="fi-annual-savings" className="block text-sm font-semibold mb-2">Annual Savings</label>
                <div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">$</span>
                  <MoneyField id="fi-annual-savings" ariaLabel="Annual savings" value={annualSavings} onChange={setAnnualSavings} className={mc} /></div>
                <p className="text-xs text-gray-500 mt-1">Increases with inflation automatically</p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div><label className="block text-sm font-semibold mb-2">Expected Rate of Return</label>
                  <div className="relative"><input type="number" value={rateOfReturn} onChange={(e) => setRateOfReturn(parseFloat(e.target.value) || 0)} className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl" /><span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500">%</span></div></div>
                <div><label className="block text-sm font-semibold mb-2">Inflation Rate</label>
                  <div className="relative"><input type="number" value={inflationRate} onChange={(e) => setInflationRate(parseFloat(e.target.value) || 0)} className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl" /><span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500">%</span></div></div>
              </div>
            </div>
          )}
          {step === 1 && (
            <div className="space-y-5">
              ...
              {hasMortgage && (
                <>
                  <div><label className="block text-sm font-semibold mb-2">Mortgage Balance</label><div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">$</span><MoneyField value={mortgageBalance} onChange={setMortgageBalance} className={mc} /></div></div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label className="block text-sm font-semibold mb-2">Interest Rate</label><div className="relative"><input type="number" value={mortgageRate} onChange={(e) => setMortgageRate(parseFloat(e.target.value) || 0)} className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl" /><span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500">%</span></div></div>
                    <div><label className="block text-sm font-semibold mb-2">Monthly Payment</label><div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">$</span><MoneyField value={monthlyPayment} onChange={setMonthlyPayment} className={mc} /></div></div>
                  </div>
                </>
              )}
            </div>
          ...
          {step === 2 && (
            <div className="space-y-5">
              ...
              <label className="flex items-center gap-2"><input type="checkbox" checked={payingForCollege} onChange={(e) => setPayingForCollege(e.target.checked)} /><span className="font-medium">Planning for college costs</span></label>
              {payingForCollege && (
                <>
                  <div className="flex justify-between items-center"><span className="text-sm text-gray-600">Kids (age & annual tuition)</span><button onClick={() => setKids([...kids, { id: Date.now(), age: 5, annualTuition: 30000 }])} className="text-sm text-emerald-600 font-medium">+ Add Child</button></div>
                  {kids.map((kid, idx) => (
                    <div key={kid.id} className="p-3 border-2 border-gray-100 rounded-xl space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div><label className="text-xs text-gray-600">Annual Tuition</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">$</span><MoneyField value={kid.annualTuition} onChange={(n) => updateKid(idx, 'annualTuition', n)} className="w-full pl-7 pr-3 py-2 border-2 rounded-lg" /></div></div>
                      </div>
                    </div>
                  ))}
                </>
          ...
          {step === 3 && (
            <div className="space-y-5">
              ...
              {buyingVehicle && (
                <>
                  <div className="flex justify-between"><span className="text-sm text-gray-600">Vehicles</span><button onClick={() => setVehicles([...vehicles, { id: Date.now(), amount: 40000, year: new Date().getFullYear() + 2 }])} className="text-sm text-emerald-600 font-medium">+ Add Vehicle</button></div>
                  {vehicles.map((v, idx) => (
                    <div key={v.id} className="p-3 border-2 border-gray-100 rounded-xl space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div><label className="text-xs text-gray-600">Amount</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">$</span><MoneyField value={v.amount} onChange={(n) => updateVehicle(idx, 'amount', n)} className="w-full pl-7 pr-3 py-2 border-2 rounded-lg" /></div></div>
