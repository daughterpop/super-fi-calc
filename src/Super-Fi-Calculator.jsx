    ...
    return (
      <div className="w-full" ref={wizardRef}>
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-xs sm:text-sm text-gray-500 text-center mb-4 max-w-2xl mx-auto">Investments only (house equity excluded). Mortgage is added to the FI target, not treated as an asset. Time to FI is the first year the portfolio stays above the inflation-adjusted target for every year after.</p>
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-5 sm:p-8">
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div><label htmlFor="fi-current-savings" className="block text-sm font-semibold mb-2">Total investments</label>
                  ...
                  <p className="text-xs text-gray-500 mt-1">{formatCompact(currentSavings)} invested today — exclude house equity</p>
                </div>
              </div>
              <div>
                ...
                <p className="text-sm text-gray-500 mt-2">Increases with inflation automatically</p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div><label className="block text-sm font-semibold mb-2">Expected Rate of Return</label>
                  <div className="relative"><input type="number" inputMode="decimal" value={rateOfReturn} onChange={(e) => setRateOfReturn(parseFloat(e.target.value) || 0)} className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl" /><span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500">%</span></div></div>
                <div><label className="block text-sm font-semibold mb-2">Inflation Rate</label>
                  <div className="relative"><input type="number" inputMode="decimal" value={inflationRate} onChange={(e) => setInflationRate(parseFloat(e.target.value) || 0)} className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl" /><span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500">%</span></div></div>
              </div>
            </div>
          )}
          {step === 1 && (
            <div className="space-y-5">
              ...
              <label className="flex items-center gap-2"><input type="checkbox" checked={hasMortgage} onChange={(e) => setHasMortgage(e.target.checked)} /><span className="font-medium">We have a mortgage</span></label>
              {hasMortgage && (
                <div className="space-y-4">
                  <div><label htmlFor="fi-mortgage-balance" className="block text-sm font-semibold mb-2">Remaining balance</label>
                    <div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">$</span>
            ...
            <div className="space-y-5">
              ...
              {payingForCollege && (
                <div className="space-y-3">
                  <div className="flex justify-between"><span className="text-sm text-gray-600">Add each child</span><button onClick={() => setKids([...kids, { id: Date.now(), age: 5, annualTuition: 30000 }])} className="text-sm text-emerald-600 font-medium">+ Add Child</button></div>
                  {kids.map((kid, idx) => (
                    <div key={kid.id} className="p-3 border-2 border-gray-100 rounded-xl space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div><label className="text-xs text-gray-600">Annual tuition</label><div className="relative"><span className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-500 text-sm">$</span>
                ...
                {otherExpenses.map((e, idx) => (
                  <div key={e.id} className="p-3 border-2 border-gray-100 rounded-xl space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div><label className="text-xs text-gray-600">Name</label><input type="text" value={e.name} onChange={(ev) => updateOtherExpense(idx, 'name', ev.target.value)} className="w-full px-3 py-2 border-2 rounded-lg" /></div>
                      <div><label className="text-xs text-gray-600">Amount</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">$</span>
                        <MoneyField ariaLabel="Expense amount" value={e.amount} onChange={(n) => updateOtherExpense(idx, 'amount', n)} className="w-full pl-7 pr-3 py-2 border-2 rounded-lg" /></div></div>
                      <div><label className="text-xs text-gray-600">Year</label><input type="number" value={e.year} onChange={(ev) => updateOtherExpense(idx, 'year', parseInt(ev.target.value) || 0)} className="w-full px-3 py-2 border-2 rounded-lg" /></div>
                    </div>
                  </div>
                ))}
              </>)}
          ...
          <div className="mt-8 flex justify-between">
            <button onClick={() => setStep(Math.max(0, step - 1))} disabled={step === 0} className="px-5 py-2.5 text-sm font-medium text-gray-600 disabled:opacity-40">Back</button>
            <button onClick={nextStep} className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl text-sm">{step === 3 ? 'See results' : 'Continue'}</button>
          </div>
        </div>
      </div>
    </div>
